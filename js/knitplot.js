...
(function() {
  var ChartParser, ErrorView, Knitplot, Library, NotificationView, Pattern, PatternEditView, PatternListView, Router, drawCircle, drawGraphic, drawLine, drawPolygon, drawRectangle, drawShape, makeGraphic, scaleAndTranslate, scaleAndTranslatePoint,
  ...
  drawPolygon = function(canvas, shape) {
    var path, polygon;
    path = ("M" + shape.polygon[0][0] + "," + shape.polygon[0][1]) + _.map(shape.polygon.slice(1), function(point) {
  ...
  scaleAndTranslate = function(shape, x, y, width, height) {
    ...
    if (shape.line) {
      ...
      return {
        polygon: _.map(shape.polygon, function(point) {
          return scaleAndTranslatePoint(point, x, y, width, height);
        }),
        style: shape.style
      };
    } else {
